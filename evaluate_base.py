"""
评估原始 Base 模型（无任何微调）的 HumanEval Pass@1
作为 SFT 的对比基准
"""
import re
import json
...
OUTPUT_FILE = "outputs/base_humaneval.jsonl"
TEMPERATURE = 0.2
MAX_NEW_TOKENS = 512

def extract_code(raw, prompt):
    # Base 模型用 chat template，输出格式不同，直接提取代码块
...
def generate_solution(model, tokenizer, prompt):
    # Base 模型直接用 chat template，不用我们自定义的格式
    messages = [
        {
            "role": "system",
